...
/**
 * Generated class for the Constructure page.
 *
...
@IonicPage()
@Component({
  selector: 'page-constructure',
  templateUrl: 'constructure.html',
})
export class Constructure {
  ...
  alldata() {
    ...
    if (obj.success) {
      ...
      this.auth.allconstructorqoute().subscribe((data) => {
        if (data.success) {
  ...
  presentProfileModal(id) {
    ...
    let profileModal = this.modalCtrl.create(Qoutemodal, { dt: this.arr, id: id, loged_in: '6' });
  ...
  search(searchEvent) {
    ...
    if (term.trim() === '' || term.trim().length < 3) {
      ...
      this.auth.constsearch(obj).subscribe(res => {
         if(res.success){
