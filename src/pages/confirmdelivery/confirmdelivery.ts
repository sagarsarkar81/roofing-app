...
/**
 * Generated class for the Confirmdelivery page.
 *
...
@IonicPage()
@Component({
  selector: 'page-confirmdelivery',
  templateUrl: 'confirmdelivery.html',
})
export class Confirmdelivery {
  ...
  ionViewDidLoad() {
    ...
    if (obj.success) {
      ...
      this.auth.confirmdeliveryproject().subscribe((data) => {
        if (data.success) {
  ...
  presentPopoverProjectListEdit(myEvent, id,constructer,gutter) {
    let obj = {
    ...
    let popover = this.popoverCtrl.create(Popquoting, { passdata: obj });
    popover.present({
      ev: myEvent
    });
  }
  ...
  search(searchEvent) {
    ...
    if (term.trim() === '' || term.trim().length < 3) {
      ...
      this.auth.confirmsearch(obj).subscribe(res => {
       if(res.success){
